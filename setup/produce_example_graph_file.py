...
os.makedirs("app/freqs_dicts", exist_ok=True)

freqs_files = os.listdir("example/data/freqs/")
all_freqs = {}

for file in freqs_files:
    print(f"Reading file: {file}")
    freqs = {}
    with gzip.open(f"example/data/freqs/{file}", 'rt') as f:
        for line in f:
...
with open(f"app/freqs_dicts/all_freqs.pickle", "wb") as f:
    pickle.dump(all_freqs, f)
print("4. Produced Pickled Freqs: app/freqs_dicts/all_freqs.pickle")

# Copy pop ratio file
shutil.copy('output/pop_counts_file.txt', 'app/pop_ratio.txt')
