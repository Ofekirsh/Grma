...
PATH_TO_CONFIG = os.environ.get("WEBSITE_CONFIG_FILE", "website-minimal-configuration.json")
print(f"Using configuration file: {PATH_TO_CONFIG}")

PATH_TO_GRIM_GRAPH = os.environ.get("GRIM_GRAPH", "graph.pickle")
...
with open(f"./freqs_dicts/all_freqs.pickle", "rb") as f:
...
def read_haps(path, race_str):
    ...
    for hla in hlas:
        ...
        for race, _ in all_freqs.items():
            if race in populations and hla in grim_graph.graph.nodes:
                freq = grim_graph.graph.nodes[hla]["freq"][populations.index(race)]
                #freq = [str(round(f, 3)) for f in freq]
                hla_and_probs[hla][race] = f"{freq:0.8e}"
            elif race in populations and hla in grim_graph.whole_graph.nodes:
                freq = grim_graph.whole_graph.nodes[hla]["freq"][populations.index(race)]
                hla_and_probs[hla][race] = f"{freq:0.8e}"
                #freq = [str(round(f,3)) for f in freq]
                #hla_and_probs[hla][race] = f"{freq:0.8e}"
